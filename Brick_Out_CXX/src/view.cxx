...
{
    // TODO: your code here
    sprites.add_sprite(ball_sprite, {(int)model.ball.top_left().x,
    (int)model.ball.top_left().y});
    sprites.add_sprite(paddle_sprite, {(int)model.paddle.x,(int)
    model.paddle.y});
    for (int i = 0; i < (int)model.bricks.size(); i++){
        sprites.add_sprite(brick_sprite, {(int)model.bricks[i].x,
      (int)model.bricks[i].y});
    }
}
