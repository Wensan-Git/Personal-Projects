...
TEST_CASE("ball moving freely")
{
    ...
    m.on_frame(0);
    expected_center += 0 * m.ball.velocity;
    CHECK(m.ball.center == expected_center);
}
...
TEST_CASE("Bouncing_around"){
    Model mod(config);
    mod.bricks.clear();
    mod.ball.center = {300, 20};
    mod.ball.radius = 5;
    mod.ball.live = true;
    mod.ball.velocity = {0 , -15};
    mod.on_frame(1);
    CHECK(mod.ball.center.y == 5);
    CHECK(mod.ball.velocity.height == -15);
    mod.on_frame(1);
    CHECK(mod.ball.velocity.height == 15);
    CHECK(mod.ball.center.y == 20);
    CHECK(mod.ball.radius == 5);
    mod.ball.center = {1004, 20};
    mod.ball.velocity = {15, -15};
    mod.on_frame(1);
    CHECK(mod.ball.velocity.width == 15);
    CHECK(mod.ball.velocity.height == -15);
    CHECK(mod.ball.center.x == 1019);
    CHECK(mod.ball.center.y == 5);
    mod.on_frame(1);
    CHECK(mod.ball.velocity.width == -15);
    CHECK(mod.ball.velocity.height == 15);
    CHECK(mod.ball.center.x == 1004);
    CHECK(mod.ball.center.y == 20);
    mod.on_frame(1);
    CHECK(mod.ball.center.x == 989);
    CHECK(mod.ball.center.y == 35);
    mod.ball.center = {1004, 748};
    mod.ball.velocity = {15, 15};
    mod.on_frame(1);
    CHECK(mod.ball.velocity.width == 15);
    CHECK(mod.ball.velocity.height == 15);
    CHECK(mod.ball.center.x == 1019);
    CHECK(mod.ball.center.y == 763);
    mod.on_frame(1);
    CHECK(mod.ball.live == false);
}

TEST_CASE("Paddle_on"){
    Model mod(config);
    mod.paddle = {100, 600, 100, 20};
    CHECK(mod.ball.live == false);
    mod.ball.center.x = 150;
    mod.ball.center.y = 594;
    mod.paddle_to(50);
    CHECK(mod.paddle.x == 50);
    CHECK(mod.ball.center.x == 100);
    CHECK(mod.ball.center.y == 594);
    CHECK(mod.ball.live == false);
}
