...
class ReluActivation:
    ...
    def backward(self, grad, lr=None):
        ...
        """
        new_grad = np.zeros((self.input_.shape[0],self.input_.shape[1]))
        #print(self.input_.shape)
        for i in range(self.input_.shape[0]):
            for j in range(self.input_.shape[1]):
                if self.input_[i][j] <= 0:
                    new_grad[i][j] = float(0)
                else:
                    new_grad[i][j] = float(1)
        return new_grad * grad
        raise NotImplementedError
...
class FullyConnected:
    ...
    def backward(self, grad, lr=0.1):
        ...
        if self.regularizer is not None:
            #print(self.regularizer.grad(self.weights))
