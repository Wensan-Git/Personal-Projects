...
func Test1(t *testing.T) {
  ...
  // expect the new primary to be the previous backup, and the previous primary (after rebooting), becomes the new backup
  ...
  
  {
    ...
    // fmt.Printf("passed first checkpoint")
    vy, _ := ck1.Get()
    // ck3 is the primary, but it never acked.
    // let ck3 die. check that ck1 is not promoted.
    // ck3 never acknowledge vy.Viewnum by pinging to the viewService with vy.Viewnum, therefore
    // the current viewNum cannot update
    for i := 0; i < DeadPings * 3; i++ {
