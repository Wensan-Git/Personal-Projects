...
type MapReduce struct {
  ...
  pendingMaps chan int
  pendingReduces chan int
  finishedMaps int
  finishedReduces int
...
func InitMapReduce(nmap int, nreduce int,
  ...
  mr.pendingMaps = make(chan int)
  mr.pendingReduces = make(chan int)
  mr.finishedMaps = 0
  mr.finishedReduces = 0
  return mr
}

// initialize MapReduce and starts the process
...
func DoReduce(job int, fileName string, nmap int,
  ...
  for i := 0; i < nmap; i++ {
    ...
    // the contents of key-value pairs from the ith map job for the job_th reduce job
    ...
    for {
      ...
      // check if the current key exists in kvs map: append the value under the current key
      _, ok := kvs[kv.Key]
      if !ok {
        kvs[kv.Key] = list.New()
      } 
  ...
  var keys []string
  // extract all the keys from all the maps for the job_th reduce job
  for k := range kvs {
    keys = append(keys, k)
  }
  sort.Strings(keys)
  p := MergeName(fileName, job)
  // create a file for the job_th reduce job, record the total number of each key in the file
...
// calls the RunMaster function on MapReduce
