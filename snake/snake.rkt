...
(define (game-score g)
  (- (* 100 (length (snake-segments (game-snake g))))
     (game-ticks g)))
...
(define (same-position? pos list)
  (local [(define (helper list accum)
            (cond [(empty? list) accum]
                  [else (helper (rest list)
                                (or (and (= (posn-x pos) (posn-x (first list)))
                                         (= (posn-y pos) (posn-y (first list))))
                                    accum))]))]
    (helper list false)))
...
(define (body-touch? g)
  (and (> (length (snake-segments (game-snake g))) 2)
              (same-position? (list-ref (snake-segments (game-snake g)) 0)
                              (rest (snake-segments (game-snake g))))))
        

(define (wall-touch? list)
  (local [(define (helper list accum)
            (cond [(empty? list) accum]
                  [else          (helper (rest list)
                                       (or (or (> (posn-x (first list)) 50)
                                               (< (posn-x (first list)) 1)
                                               (> (posn-y (first list)) 50)
                                               (< (posn-y (first list)) 1))
                                           accum))]))]
    (helper list false)))

(define (obstacle-touch-helper list g)
  (cond [(empty? list) false]
        [else (or (same-position? (first list)
                                  (snake-segments (game-snake g)))
                  (obstacle-touch-helper (rest list) g))]))

(define (obstacle-touch? g)
  (obstacle-touch-helper (game-obstacles g)
                         g))


(define (game-over? g)
  (or (body-touch? g)
      (wall-touch? (snake-segments (game-snake g)))
      (obstacle-touch? g)))
...
(define (direction-addition g)
  (cond [(equal? (snake-heading (game-snake g)) 'up)
         (make-posn (posn-x (first (snake-segments (game-snake g))))
                    (+ 1
                       (posn-y (first (snake-segments (game-snake g))))))]
        [(equal? (snake-heading (game-snake g)) 'down)
         (make-posn (posn-x (first (snake-segments (game-snake g))))
                    (- (posn-y (first (snake-segments (game-snake g))))
                       1))]
        [(equal? (snake-heading (game-snake g)) 'right)
         (make-posn (+ 1
                       (posn-x (first (snake-segments (game-snake g)))))
                    (posn-y (first (snake-segments (game-snake g)))))]
        [(equal? (snake-heading (game-snake g)) 'left)
         (make-posn (- (posn-x (first (snake-segments (game-snake g))))
                       1)
                    (posn-y (first (snake-segments (game-snake g)))))]))
...
(define (head-addition g)
  (cons (direction-addition g)
        (snake-segments (game-snake g))))
...
(define (advance-game g)
  (make-game (make-snake (snake-heading (game-snake g))
                         (cond [(equal? (same-position? (direction-addition g)
                                                        (game-food g))
                                        false)
                                (reverse (rest (reverse (head-addition g))))]
                               [else    (head-addition g)]))
             (remove (direction-addition g)
                     (game-food g))
             (game-obstacles g)
             (+ (game-ticks g) 1)))
